...
실행 흐름:
  ...
  [직렬] P3  process_audio             — 2-pass 교정 + 침묵 구간 + verifier context 생성
...
def process_audio(
    ...
    if seg_ok and silences_ok and by_scene_ok:
        ...
        log.warning(
            ...
            f"by_scene={by_scene_path.exists()})"
        )
    ...
    # [3B-4] verifier context 구조 생성
    print("  [3B-4] verifier context 구조 생성...")
    t0 = time.time()
    annotated_segments: list[dict] = segments_clean
    ...
    if slide_ranges:
        ...
        group_segments_by_context(segments_clean)
        scenes_structure = None
    ...
    print(f"    ✓ context 구조 생성  ({time.time()-t0:.1f}초)")
...
def save_scene_structure(args, audio_result: dict, output_dir: Path) -> dict:
    ...
    def _has_context_schema(path: Path) -> bool:
        try:
            with open(path, "r", encoding="utf-8") as f:
                payload = json.load(f)
    ...
    if not slide_ranges:
        if by_scene_path.exists() and by_scene_path.stat().st_size > 0 and _has_context_schema(by_scene_path):
            return {
    ...
    scenes_payload = json.loads(json.dumps(scenes_structure))
    ...
    _save_json(by_scene_path, _scene_payload(scenes_payload))
...
예시:
  ...
  python main.py --input input/lecture.mp4 --debug
