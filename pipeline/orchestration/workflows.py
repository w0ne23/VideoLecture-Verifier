# 전처리+검증 전체 파이프라인 실행 흐름과 pipeline_timings.json 기록 관리
import json
import time
...
class _TimingsDict(dict):
    """각 stage 함수는 캐시된 산출물을 발견해 건너뛸 때 elapsed=0.0을 반환

    run_pipeline이 실행마다 기존 timings.json 값을 이어받게 되면서, 이 0.0을
    그대로 덮어쓰면 실제로 그 stage가 돌았을 때 기록된 소요 시간이 스킵될 때마다
    지워짐, 새 값이 0.0이고 기존에 0이 아닌 값이 있으면 갱신을 무시해서, 실제로
    다시 실행된 stage의 시간만 갱신되고 스킵된 stage는 마지막 실측치를 유지
    """
...
# 전처리+검증 전체 파이프라인 실행, job_type에 따라 전처리를 새로 돌리거나 이전 산출물을 재사용
def run_pipeline(args, progress_callback=None, *, helpers):
    ...
    # verify_only처럼 이전 실행의 산출물을 재사용하는 job은 전처리를 다시 돌리지
    # 않음, 그렇다고 이전 실행이 기록해 둔 전처리 소요 시간까지 지우면 안 되므로,
    # 기존 timings.json이 있으면 그 내용을 이어받아 이번 실행에서 갱신되는 stage만
    # 덮어쓰고 나머지는 그대로 보존. run_history에는 실행할 때마다(예: 전처리
    # 1회 + verify_only 1회) 각각의 소요 시간이 항목으로 쌓임
    ...
    # 현재 진행 상태를 pipeline_timings.json에 원자적으로(tmp 파일 작성 후 rename) 기록
    ...
    if job_type == helpers.JOB_TYPE_VERIFY_ONLY:
        ...
        # manifest(`{stem}_preprocess_result.json`)에서 preprocess_result를 복원
        # merged_clean.json을 손으로 편집해 검증만 다시 태우고 싶을 때(예: 오류 주입
        # 테스트) 전처리 API 비용을 반복하지 않기 위한 경로
