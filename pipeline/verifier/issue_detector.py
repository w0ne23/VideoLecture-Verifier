...
이 단계는 최종 사실 판정이 아니라, 후속 issue classification과 crosscheck가 다시 볼 만한 **오류 후보**를 고르는 단계입니다.
여기서 오류 후보란, 제공된 발화 문맥을 반영한 최종 전달 의미에 학생이 잘못 배울 수 있는 구체적인 오류가 실제로 남아 있을 가능성이 있는 claim입니다.
단순히 외부 확인이 가능하거나, 출처가 없거나, 슬라이드에 직접 적혀 있지 않다는 이유만으로 후보로 출력하지 마세요.
...
모든 모델에 동일하게 적용되는 출력 threshold는 아래 응답 지침에 제시된 값을 따르세요.
