...
# judge 단계 기본 모델의 저렴한 변형이 지정되면 상위 모델로 승격
...
# 모델 스펙 끝에 붙은 #think/#nothink를 떼어내 (원본, on/off)로 반환
...
# Anthropic Structured Outputs(output_config.format) 요청 kwargs 구성
...
def _call_llm(
    ...
    """모델에 무관한 단일 LLM 호출, (응답 텍스트, usage) 반환"""
    ...
    # 웹 UI에서 선택한 프로필이 있으면 그걸 우선 사용, 아래 provider 분기는
    # 프로필 없이 시작된 job을 위한 호환 폴백으로 유지
